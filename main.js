   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +24160xx")))
            process.exit(0)
         }
      } else {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2416033xx")))
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ ALIEN XV ᵇᵘᵍ ᵇᵒᵗ ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: DADDY`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: DADDY-IGWE`))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @DADDY'S `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 24160338758`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: IGWE Sir\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
